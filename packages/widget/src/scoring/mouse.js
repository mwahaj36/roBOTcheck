...
function dist(p1,p2){
    return Math.sqrt((Math.pow((p2.x-p1.x),2))+(Math.pow((p2.y-p1.y),2)))//distance eucilidean
}
...
function pearsonCorrelation(a,b){//pearsonCorrelation used for momentum checks
    let aAvg=avg(a);
    let bAvg=avg(b);
...
function velocityProfile(points){
    ...
    for(let i=1;i<points.length;i++){
        ...
        let distance=dist(p1,p2); //use helper to get distance
        let time=points[i].t-points[i-1].t;//calc time
        time=Math.max(time,1);//prevent division by 0
        let speed=distance/time;

        speedArray.push(speed)//add to array
    }
    return speedArray;
}

function scoreFittsLaw(points,clickTarget){  //check if user slows down on second part of path
...
function microCorrections(points){
    ...
    for(let i=2;i<points.length;i++){   
        ...
        let delta=(Math.abs(prevAngle-currAngle))*180/Math.PI; //degrees of correction

        delta>10 && delta<170 ? reversals.push(delta):null; //if a valid change then push
    }
    if (reversals.length<2){ //not enough
            return 20;
    }
    let angleStdev=stdev(reversals);//to check for robotic reversals
    if (angleStdev<8){
            return 15;
    }
    else{
